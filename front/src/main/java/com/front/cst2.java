...
public class cst2 extends Activity {
...
//				tr1.setVisibility(View.INVISIBLE);
	...
	//	Button calci=(Button)findViewById(R.id.button1);
	...
	//	conclusion.setVisibility(View.INVISIBLE);
		...
		clear.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				myintent.setClass(getApplicationContext(), cst2.class);
			...
			//	  double gdb[]=new double[row];
                ...
                edi.putString("rows", row+"");
		        	...
		        	//	edi.putString("col1"+(i+1),	freq[i]+"");
			            edi.putString("col1"+(i+1), vi[i]+"");
			            edi.putString("col2"+(i+1), vo[i]+"");
			            edi.putString("col3"+(i+1), res1[i]+"");
		...
		*/
		...
		conclusion.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				final Dialog dialog = new Dialog(cst2.this);
    		...
    		public void onClick(View v) {
  			...
  			//	  double gdb[]=new double[row];
  				...
  				//	freq[i]=Double.parseDouble(evalues[i][0].getText().toString());
  						...
  						vi[i]=Double.parseDouble(evalues[i][0].getText().toString());
  						vo[i]=Double.parseDouble(evalues[i][1].getText().toString());
  						p[i]=Double.parseDouble(evalues[i][2].getText().toString());
      	       ...
      	       //     bundle1.putDoubleArray("gdb",gdb);
    				...
    				//	heads[1]=new HeaderHTM("V<sub>i</sub>", subheads, 0, 0);
    					...
    					heads[3]=new HeaderHTM("Phase", subheads, 0, 0);
    				...
    				SharedPreferences edi2=getSharedPreferences("exp5", 0);
    	            
    	            
    	            for(int i=0;i<row;i++){
    	            	filetable[i][0]=(i+1)+"";
    	       //     filetable[i][1]=edi.getString("col1"+(i+1),	"");
    	            filetable[i][1]=edi2.getString("col1"+(i+1), "");
    	            filetable[i][2]=edi2.getString("col2"+(i+1), "");
    	            filetable[i][3]=edi2.getString("col3"+(i+1), "");
...
//    				String filecalculation[]=new String[1];
    				...
    				try
    				{
    					...
    					File folder1 = new File("/sdcard/cs2/tab/");
    					if(!folder1.exists())
    						folder1.mkdir();
    					else
    					{
    				...
    				String file="/sdcard/cs2/tab/"+rno+"_expt1.html";
...
//    				htm.write_calc(filecalculation);
      			...
      			String []ss=new String[1];
    				...
    				//String []ss=new String[1];
    				Compress c =new Compress(ss, "/sdcard/cs2/tab/"+rno+"_expt1.zip"); 
    				c.zip();
    				//<<<<<<<<<<<<<UPLOADING>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

    				AlertDialog.Builder builder = new AlertDialog.Builder(cst2.this);
